...
The platform's **expression engine** — a [JSONLogic](https://jsonlogic.com)
evaluator. Used for Param `showIf` visibility (and, later, output→input mapping
in workflows). JSONLogic was chosen over a bespoke mini-language in the ROADMAP.
...
`var`, `missing` · `==` `===` `!=` `!==` · `<` `<=` `>` `>=` (with the chained
"between" form, e.g. `{ "<": [1, x, 10] }`) · `and` `or` `!` `!!` · `if` /
`?:` (elseif chains) · `in` (string or array) · `+` `-` `*` `/` `%`.

Unknown operators throw `ExprError`. JSONLogic truthiness applies (an empty
array is falsy).
